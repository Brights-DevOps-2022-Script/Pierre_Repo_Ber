    ...
    stages {
        ...
        stage('Deploy') {
            ...
            steps{
                ...
                sh 'kubectl --kubeconfig=$KUB_CONF get deployments -n pierre-space-second'
                sh 'kubectl --kubeconfig=$KUB_CONF expose deployment nginx-deployment-pierre --type=LoadBalancer --name=load-balancer -n pierre-space-second'
            }
        }
    }   
}